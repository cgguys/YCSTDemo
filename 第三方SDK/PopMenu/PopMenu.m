...
- (void)setup {
    ...
    _realTimeBlur.willDismissBlurViewCompleted = ^(void) {
        ...
        if (weakSelf.willCancelBlock) {
            weakSelf.willCancelBlock();
        }
    };
...
- (void)showMenuAtView:(UIView *)containerView startPoint:(CGPoint)startPoint endPoint:(CGPoint)endPoint {
    [self showMenuAtView:containerView startPoint:startPoint endPoint:endPoint layoutStyle:PopMenuLayoutStyleGrid];
}

- (void)showMenuAtView:(UIView *)containerView startPoint:(CGPoint)startPoint endPoint:(CGPoint)endPoint layoutStyle:(PopMenuLayoutStyle)layoutStyle {
    if (self.isShowed) {
        return;
    }
    self.startPoint = startPoint;
    self.endPoint = endPoint;
    self.menuLayoutStyle = layoutStyle;
...
- (CGRect)getFrameWithItemCount:(NSInteger)itemCount
    ...
    if (self.menuLayoutStyle == PopMenuLayoutStyleGrid) {
        originX = (index % perRowItemCount) * (itemWidth + paddingX) + paddingX + (page * CGRectGetWidth(self.bounds));
        originY = ((index / perRowItemCount) - perColumItemCount * page) * (itemHeight + paddingY) + paddingY;
        CGRect itemFrame = CGRectMake(originX, originY + insetY, itemWidth, itemHeight);
        return itemFrame;
    } else if(self.menuLayoutStyle == PopMenuLayoutStyleArc){
        NSInteger centerIndex = self.items.count/2;
        
        NSInteger diff =  ABS(centerIndex - index);
        NSLog(@"%@", @(diff));
        
        originY = self.bounds.size.height - itemHeight - 120 + diff * 20;
        originX = index * self.bounds.size.width/self.items.count;
